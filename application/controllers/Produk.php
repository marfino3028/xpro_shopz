...
class Produk extends CI_Controller {
	...
	function perusahaan(){
		if (reseller($this->session->id_konsumen)==''){
			echo $this->session->set_flashdata('message', '<div class="alert alert-danger"><center>Maaf, halaman tersebut khusus untuk Reseller/Pelapak,..</center></div>');
			redirect('main');
		}else{
	...
	function thirdsubkategori(){
		$cek = $this->model_app->edit('rb_kategori_produk_sub_third',array('kategori_seo_sub_third'=>$this->uri->segment(3)))->row_array();
		
		$jumlah= $this->model_app->view_where('rb_produk',array('id_kategori_produk_sub_third'=>$cek['id_kategori_produk_sub_third']))->num_rows();
		$config['base_url'] = base_url().'produk/thirdsubkategori/'.$this->uri->segment(3);
		$config['total_rows'] = $jumlah;
		$config['per_page'] = 12; 	
		if ($this->uri->segment('4')==''){
			$dari = 0;
		...
		if (is_numeric($dari)) {
			$data['title'] = "Third Subkategori / $cek[nama_kategori_sub_third]";
			$data['judul'] = "Third Subkategori / $cek[nama_kategori_sub_third]";
			$data['description'] = description();
			$data['keywords'] = keywords();
			$this->pagination->initialize($config);
			$data['record'] = $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='0' AND a.id_produk_perusahaan='0' AND a.id_kategori_produk_sub_third='$cek[id_kategori_produk_sub_third]' AND a.aktif='Y' ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			$this->pagination->initialize($config);
			$this->template->load(template().'/template',template().'/reseller/view_kategori_all',$data);
		}else{
			redirect('main');
	...
	function keranjang(){
		...
		// return var_dump([$this->input->post('duplicate_id'), $this->session->idp]);
		$id_produk = cetak($this->input->post('duplicate_id'));
		// $id_produk = cetak($this->session->idp);
		...
		if (isset($_POST['update'])){
			...
			foreach ($keranjang->result_array() as $row){
				...
				$cek_minimumx = $this->db->query("SELECT nama_produk, minimum, satuan FROM rb_produk where id_produk='".cetak($this->input->post('idp'.$i))."'")->row_array();
				...
				$dataa = array('jumlah'=>$qtyx,
							  'keterangan_order'=>cetak($this->input->post('keterangan'.$i)).$variasi_simpan);
		...
		if ($id_produk!=''){
			if ($stok < $qty){
				...
				if ($reseller_cek->num_rows()<'3'){
					...
					$data['description'] = description();
					$data['keywords'] = keywords();
					echo $this->session->set_flashdata('message', '<div class="alert alert-info"><center>Ingat, 1 Transaksi maksimal ke 3 Toko saja,..</center></div>');
					redirect('produk/keranjang');
				}
			}
		}else{
	...
	function sopir_get_cost(){
		$kurir = $this->input->post('kurir_pengiriman',TRUE);
		$kecamatan_dari = $this->input->post('kecamatan_dari',TRUE);
		$kecamatan_tujuan = $this->input->post('kecamatan_tujuan',TRUE);
		...
		if (isset($_POST['submit'])){
			...
			if ($this->input->post('metode')=='saldo'){
				if (saldo(reseller($this->session->id_konsumen),$this->session->id_konsumen)>=(($ttal['total']-$ttal['diskon_total'])+$tong['ongkir'])){
					...
					$data = array('proses'=>'1');
					$where = array('id_penjualan'=>$idp,'id_pembeli'=>$this->session->id_konsumen,'status_pembeli'=>'konsumen');
...
Butuh Metode Pembayaran Lainnya? klik disini : ".base_url()."konfirmasi/bayar?inv=$kode_transaksi";
            ...
            $cek_onl = $this->db->query("SELECT * FROM rb_penjualan where kode_kurir!='0' AND kode_transaksi='$kode_transaksi'");
    		if ($cek_onl->num_rows()>=1){ // Cek Transaksi Online
    			if ($this->input->post('metode')=='saldo'){
    				if (saldo(reseller($this->session->id_konsumen),$this->session->id_konsumen)>=$total_tagihan_akhir){
    					// Pembayaran Sukses
...
Hai *$kons[nama_lengkap]*, Terima kasih telah berbelanja di $iden[url], pembayaran anda untuk orderan ini telah kami terima Dan kami telah menginformasikan kepada seller untuk segera diproses.";
    					...
    					$message_sukses  = "<html><body><img src='".base_url()."asset/logo/$logo[gambar]' style='height:87px'><br>
    	        						<span style='font-size:20px'>Transaksi #$kode_transaksi Sukses</span><br>
    					...
    					$this->model_app->wa(format_telpon($kons['no_hp']),$isi_pesan_sukses);
    				}else{
			...
			foreach ($penjual_notifx->result_array() as $penx) {
				...
				$this->model_app->wa(format_telpon($penx['no_telpon']),$isi_pesan_penjual);

				$konsumx = $this->db->query("SELECT email FROM rb_konsumen where id_konsumen='$penx[id_konsumen]'")->row_array();
	...
	function checkouts(){
		...
		if (isset($_POST['submit'])){
			...
			$data['record'] = $this->db->query("SELECT a.*, b.nama_produk, b.gambar, b.produk_seo, c.nama_reseller, GROUP_CONCAT(d.nama SEPARATOR '||') as nama, GROUP_CONCAT(d.variasi SEPARATOR '||') as variasi FROM rb_penjualan_temp a JOIN rb_produk b ON a.id_produk=b.id_produk JOIN rb_reseller c ON b.id_reseller=c.id_reseller 
	...
	function read_query(){
		$data = $this->db->query("SELECT a.*, b.nama_produk, b.gambar, b.produk_seo, c.nama_reseller, GROUP_CONCAT(d.nama SEPARATOR '||') as nama, GROUP_CONCAT(d.variasi SEPARATOR '||') as variasi FROM rb_penjualan_temp a JOIN rb_produk b ON a.id_produk=b.id_produk JOIN rb_reseller c ON b.id_reseller=c.id_reseller 
									LEFT JOIN rb_produk_variasi d ON a.id_produk=d.id_produk
