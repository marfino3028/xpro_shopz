...
class Members extends CI_Controller {
	...
	function trx_pulsa(){
		...
		$data['row'] = $this->model_reseller->profile_konsumen($this->session->id_konsumen)->row_array();
		$data['pulsa'] = $this->db->query("SELECT * FROM `rb_pembelian_pulsa` where id_reseller='".reseller($this->session->id_konsumen)."' ORDER BY waktu_pembelian DESC LIMIT $dari,$config[per_page]");
		$this->pagination->initialize($config);
	...
	function selesai_belanja(){
		if (isset($_POST['submit'])){
			...
			$data1 = array('kurir'=>cetak($this->input->post('kurir')),
						   'service'=>cetak($this->input->post('service')),
						   'ongkir'=>cetak($this->input->post('ongkir')));
	...
	function tambah_produk(){
        ...
        if (isset($_POST['submit'])){
			...
			if ($status>=1){
				...
				if ($cek_produk->num_rows()>=$cekpa['max_produk']){
					...
					$data = array('id_kategori_produk'=>cetak($this->input->post('a')),
								...
								'id_kategori_produk_sub_third'=>cetak($this->input->post('aaa')),
								'id_reseller'=>reseller($this->session->id_konsumen),
								...
								'gambar'=>$fileName,
								'tentang_produk'=>cetak($this->input->post('fff')),
            ...
            $data['record'] = $this->model_app->view_ordering('rb_kategori_produk','nama_kategori','ASC');
	...
	function kupon_save(){
		if (cetak($this->input->post('a'))!=''){
			...
			if ($this->input->post('id_kupon')==''){
				...
				$where = array('id_kupon' => $this->input->post('id_kupon'));
    ...
    function delete_produk(){
		cek_session_members();
		verifikasi(reseller($this->session->id_konsumen));
        $id = array('id_produk' => $this->uri->segment(3),'id_reseller'=>reseller($this->session->id_konsumen));
	...
	function detail_penjualan(){
		...
		if (isset($_POST['submit'])){
				...
				$iden = $this->model_app->view_where('identitas',array('id_identitas'=>'1'))->row_array();
				$rows = $this->db->query("SELECT * FROM rb_penjualan a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_penjualan='".cetak($this->uri->segment(3))."'")->row_array();
				...
				$this->model_app->wa(format_telpon($rows['no_hp']),$isi_pesan_pembeli);
				...
				echo kirim_email($subject_notif,$message_notif,$rows['email']);
	...
	function tambah_withdraw(){
		...
		if (isset($_POST['submit'])){
			if (reseller($this->session->id_konsumen)!=''){
				$id_reseller = reseller($this->session->id_konsumen);
				$akun = 'reseller';
				$rows = $this->db->query("SELECT b.* FROM rb_reseller a JOIN rb_konsumen b ON a.id_konsumen=b.id_konsumen where a.id_konsumen='".$this->session->id_konsumen."'")->row_array();
			}else{
				$id_reseller = $this->session->id_konsumen;
				$akun = 'konsumen';
				$rows = $this->db->query("SELECT * FROM rb_konsumen where id_konsumen='".$this->session->id_konsumen."'")->row_array();
			}
			...
			if (cetak($this->input->post('jenis'))=='debit'){
				...
				if ($nominal>=config('withdraw_min')){
					if (saldo(reseller($this->session->id_konsumen),$this->session->id_konsumen)>=$nominal){
						if ($nominal>0){
							...
							$iden = $this->model_app->view_where('identitas',array('id_identitas'=>'1'))->row_array();
							$adm = $this->model_app->view_where('users',array('level'=>'admin'))->row_array();
							...
							$this->model_app->wa(format_telpon($rows['no_hp']),$isi_pesan_konsumen);
							$this->model_app->wa(format_telpon($adm['no_telp']),$isi_pesan_admin);
				...
				if (clean_rupiah(cetak($this->input->post('b')))>0){
					...
					// Kirim Notifikasi Withdraw ke admin
					$rek = $this->model_app->view_where('rb_rekening',array('id_rekening'=>cetak($this->input->post('a'))))->row_array();
					$iden = $this->model_app->view_where('identitas',array('id_identitas'=>'1'))->row_array();
					$adm = $this->model_app->view_where('users',array('level'=>'admin'))->row_array();
...
Jika sudah Transfer silahkan konfirmasi disini agar bisa diproses lebih cepat, Terima kasih...";
					$this->model_app->wa(format_telpon($rows['no_hp']),$isi_pesan_wa_deposit);
					...
					echo kirim_email($subject_notif,$message_notif_deposit,$rows['email']);	
	...
	function detail_keuangan(){
		...
		if (isset($_POST['upload'])){
			...
			$data['row'] = $this->db->query("SELECT a.*, b.keterangan as ket FROM rb_withdraw a LEFT JOIN rb_konsumen_detail b ON a.id_rekening_reseller=b.id_konsumen_detail where id_withdraw ='".$this->input->post('id')."'")->row_array();
			$this->load->view(template().'/reseller/view_withdraw_detail.php',$data);
		}
	}
}
