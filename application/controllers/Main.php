...
class Main extends CI_Controller {
	public function email(){
		echo kirim_email('Coba Kirimkan Email','Ini adalah isi Pesan yang dikirim...','rohman.nurafan13@gmail.com');
    ...
    function change_matauang(){
		$matauang = $this->input->post('matauang');
		$this->session->set_userdata(['matauang' => $matauang]);
		$this->session->userdata('matauang');
	}
	...
	function proses(){
		...
		if (isset($_POST['submit'])){
			...
			if (saldo(reseller($this->session->id_konsumen))>=$harga){
				...
				$data = array(
					...
					'phone' => $this->input->post('tujuan'), // nohp pembeli
					'no_meter_pln' => '', // khusus untuk pembelian token PLN prabayar
					'api_trxid' => reseller($this->session->id_konsumen).''.date('YmdHis'), // ID transaksi dari server Anda. (tidak wajib, maks. 25 karakter)
				...
				if ($trx->success=='true'){
					$keterangan = reseller($this->session->id_konsumen).'-'.date('YmdHis').'|'.$ex[0].'|'.$this->input->post('tujuan');
