...
require APPPATH.'libraries/phpmailer/src/SMTP.php';
    ...
    function cek_session_members(){
        ...
        if ($session != 'konsumen'){
          echo $ci->session->set_flashdata('message', '<div class="alert alert-danger"><center>Please log in to access the page!</center></div>');
          redirect('auth/login');
        }
    }
    ...
    function cekmatauang(){
        $ci = & get_instance();
        $session = $ci->session->userdata('matauang');
        return $session;
    }

    function idr2hkd($harga,$dollar){
        $curl = curl_init();
        ...
        return number_format(round($res));
...
function cek_terakhir($datetime, $full = false) {
    ...
    function saldo($id_reseller,$id_konsumen){
        $ci = & get_instance();
        $sop = $ci->db->query("SELECT id_sopir FROM rb_sopir where id_konsumen='$id_konsumen'")->row_array();
        ...
        $tarik = $ci->db->query("SELECT sum(nominal) as total FROM rb_withdraw WHERE id_reseller='$id_reseller' AND status!='Batal' AND transaksi='debit' AND akun='reseller'")->row_array();
        $tarikx = $ci->db->query("SELECT sum(nominal) as total FROM rb_withdraw WHERE id_reseller='$id_konsumen' AND status!='Batal' AND transaksi='debit' AND akun='konsumen'")->row_array();
        $deposit = $ci->db->query("SELECT sum(nominal) as total FROM rb_withdraw WHERE id_reseller='$id_reseller' AND status='Sukses' AND transaksi='kredit' AND akun='reseller'")->row_array();
        $fee = $ci->db->query("SELECT sum(nominal) as total FROM rb_withdraw WHERE id_reseller='$id_konsumen' AND status='Sukses' AND transaksi='kredit' AND akun='konsumen'")->row_array();
        ...
        $pulsa = $ci->db->query("SELECT sum(total) as total FROM rb_pembelian_pulsa WHERE id_reseller='$id_reseller'")->row_array();
        ...
        return ((((($penjualan_perusahaan['total']-($tarik['total']+$tarikx['total']))+$deposit['total'])-$pulsa['total'])+$sopir['total'])+$fee['total']+$ongkir['ongkir'])-$kupon['diskon'];
    }
    ...
    function cek_status_payment($id_reseller){
        ...
        if ($rowp['status']=='Y'){
            ...
            return "<span style='color:red'>PENDING, <b>$rowp[nama_paket]</b></span>";
        }else{
            return "<span style='color:red'>Reseller Free</span>";
        }
    }
