<header class="header header--Motore header--Motore-product" data-sticky="true"><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
...
?>
    <nav class="navigation--Motore-product">
        <?php 
            if (stok($record['id_reseller'],$record['id_produk'])<=0){ 
                echo "<a style='color:#a7a7a7; background-color:#e2e2e2' class='ps-btn ps-btn--black rounded-0 add-to-cart-empty'>Add to Cart</a>";
            }else{
                echo "<a style='color:#fff' id='$record[id_produk]' class='ps-btn ps-btn--black rounded-0 add-to-cart'>Add to Cart</a>";
            }
        ?>

        <input class="form-control" type="hidden" value='1' name='qty'>
        <button type='submit' name='beli' class="ps-btn btn-block rounded-0">Buy Now</button>
    </nav>
</form>
