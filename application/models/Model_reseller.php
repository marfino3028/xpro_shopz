...
class Model_reseller extends CI_model{
    ...
    function menunggu_pembayaran($id,$level){
        return $this->db->query("SELECT a.*, b.*, z.pembayaran FROM rb_penjualan_otomatis z JOIN `rb_penjualan` a ON z.kode_transaksi=a.kode_transaksi JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen where z.pembayaran is NULL AND a.status_penjual='$level' AND a.id_penjual='$id' ORDER BY a.id_penjualan DESC");
    }
    ...
    function profile_konsumen($id){
        return $this->db->query("SELECT a.id_konsumen, a.username, a.nama_lengkap, a.email, a.jenis_kelamin, a.tanggal_lahir, a.tempat_lahir, a.alamat_lengkap, a.kecamatan_id, a.no_hp, a.tanggal_daftar, a.referral_id, b.kota_id, b.nama_kota as kota, c.provinsi_id, c.nama_provinsi as propinsi, a.foto FROM `rb_konsumen` a LEFT JOIN rb_kota b ON a.kota_id=b.kota_id LEFT JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id where a.id_konsumen='$id'");
    }
